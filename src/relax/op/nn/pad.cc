...
InferLayoutOutput PadInferLayout(const Call& call, 
  ...
  // return InferLayoutOutput({ret_data, ret_pad_value}, {ret_data}, Attrs(params));
}
...
StructInfo InferStructInfoPad(const Call& call, const BlockBuilder& ctx) {
  ...
  TensorStructInfo data_sinfo = infos[0];
   ...
   ICHECK_EQ(data_shape->values.size(), attrs->pad_width.size()) << "Data shape and padding should be the same size";

   Array<PrimExpr> padded_shape;
   for(size_t i = 0; i < data_shape->values.size(); i++) {
     padded_shape.push_back(data_shape->values[i] + attrs->pad_width[i][0] + attrs->pad_width[i][1]);
   }
